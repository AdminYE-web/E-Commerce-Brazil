...
@section('title', 'Edit Quotation | Indigo Admin')
...
@php
    $existingItems = $quotation->items->map(function ($item) {
        return [
            'product_id' => $item->product_id,
            'quantity' => $item->quantity,
            'selected_option_ids' => $item->options->pluck('option_id')->filter()->values()->toArray(),
        ];
    })->values();
@endphp

<form action="{{ route('admin.quotations.update', $quotation->quotation_id) }}" method="POST" id="quotation-form">
    @csrf
    @method('PUT')
    ...
    <div class="table-card quotation-page-card">
        <div class="table-header">
            <div>
                <div class="table-title">{{ request()->cookie('dev') == '1' ? 'Edit Quotation' : '引用編集' }}</div>
                <div class="showing-text">{{ $quotation->quotation_no }}</div>
            </div>

            <a href="{{ route('admin.quotations.show', $quotation->quotation_id) }}" class="btn-outline">
                {{ request()->cookie('dev') == '1' ? 'Back to Quotation' : '引用に戻る' }}
            </a>
        </div>
        ...
        <div class="quotation-form-grid">
            ...
            <div class="quotation-form-group quotation-form-full">
                ...
                <textarea name="customer_address" rows="3">{{ old('customer_address', $quotation->customer_address) }}</textarea>
            </div>
    ...
    <div><strong>{{ request()->cookie('dev') == '1' ? 'Grand Total:' : '合計:' }} ¥<span id="quotationGrandTotal">0.00</span></strong></div>
        ...
        <div class="quotation-actions">
            <button type="submit" class="quotation-save-btn">
                {{ request()->cookie('dev') == '1' ? 'Update Quotation' : '引用を更新' }}
            </button>
    ...
    <div class="quotation-item-box" data-item-index="__INDEX__">
        <div class="quotation-form-grid">
            ...
            <div class="quotation-form-group">
                ...
                <input type="number" name="items[__INDEX__][quantity]" class="quotation-qty" value="1" min="1" required>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const existingItems = @json(old('items') ? [] : $existingItems);
...
if (discountInput) {
    ...
    function addItem(existingItem = null) {
        ...
        const itemBox = wrapper.querySelector('.quotation-item-box[data-item-index="' + currentIndex + '"]');

        if (existingItem) {
            const productSelect = itemBox.querySelector('.quotation-product-select');
            const qtyInput = itemBox.querySelector('.quotation-qty');

            productSelect.value = existingItem.product_id;
            qtyInput.value = existingItem.quantity || 1;

            loadProductOptions(productSelect, existingItem.selected_option_ids || []);
        }
    ...
    function loadProductOptions(select, selectedOptionIds = []) {
            ...
            .then(data => {
                ...
                data.groups.forEach(group => {
                    ...
                    group.options.forEach(option => {
                        ...
                        const checked = selectedOptionIds.map(Number).includes(Number(option.option_id))
                            ? 'checked'
                            : '';
...
calculateQuotationSummary();
            ...
            .catch(error => {
                console.error(error);
                optionsArea.innerHTML = '<div style="color:#dc2626;">Unable to load product options.</div>';
            });
    }
    ...
    function calculateItemTotal(itemBox) {
        ...
        checkedOptions.forEach(input => {
            ...
            const isRuleOption = matchedRule && matchedRule.option_ids.map(Number).includes(optionId);
